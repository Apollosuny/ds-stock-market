...
class MERGE_OLAP:
    ...
    def transform_dim(self, last_timestamp=None):
        if last_timestamp:
            self.logger.info(
                f"Processing data from last timestamp: {last_timestamp}"
            )
        else:
    ...
    def transform_fact(self, last_timestamp=None):
        if last_timestamp:
            self.logger.info(
                f"Processing data from last timestamp: {last_timestamp}"
            )
        ...
        self.logger.info("Performing build fact table.")
        # fact table
